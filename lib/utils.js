    ...
    for (i=0; i< data.length; i++) {
        ...
        for (j=0; j< data.length; j++) {
            ...
            for (k=0;k<data[0].length;k++) {
                if (state[k] == 1) {
                    //Euclidean distance
                    dist = dist + Math.pow(Math.abs(data[i][k]-ranges[k][0])/ranges[k][2] -Math.abs(data[j][k]-ranges[k][0])/ranges[k][2],2);
                    // Manhattan distance
        ...
        for (i = 0; i < MDSdata.length; i++) {
            for (j = 0; j < MDSdata.length; j++) {
                ...
                if (min_dist > dist && dist >= 0) {
                    ...
                    MDSdata[j][0] = x.invert(x(MDSdata[i][0]) + (2*radius - dist)*numeric.cos([angle])[0]);
                    MDSdata[j][1] = y.invert(y(MDSdata[i][1]) + (2*radius -dist)*numeric.sin([angle])[0]);
...
// Put data in circles
        ...
        .on("mouseover", function (d,i) {
            ...
            d3.selectAll("circle")
                ...
                .style("opacity", function () { return (this === circleUnderMouse) ? "0.8":"0.3";});
            d3.select(this)
                .transition().delay(1000).duration(20)
            ...
            tooltip.transition().delay(500).duration(20).style("visibility","visible");
          }})
        .on("mousemove", function () {
            tooltip.style("top", (event.pageY - 10) + "px").style("left", (event.pageX + 10) + "px");
        })
        .on("mouseout", function (d,i) {
            d3.selectAll("circle").transition().duration(1).style("opacity",function (d) {
              if (filter(d.info)) { return 0.8;}
              else {return 0;}
            });
            d3.select(this).transition().duration(1).attr("stroke-width",0);
            tooltip.transition().delay(3000).duration(1).style("visibility", "hidden");
        })
    ...
    svg.on("mousemove", function (){
        ...
        .attr("cy", function(d) { return d.fisheye.y ; })
            .attr("r", function(d) { return d.fisheye.z *3.0; });
    });
    ...
    d3.select("#writedata").on("click", function() {
        ...
        text+= '<br>';
        d3.selectAll('circle').each( function (d,i){
            if ( d3.select(this).classed("selected") && d3.select(this).style("opacity") != 0) {
                for (i = 0; i < leng - 1; i++) {
...
// change geometry to geometryAtoms here
        ...
        geometryBonds.translate( offset.x, offset.y, offset.z );
            for ( var i = 0; i < geometryAtoms.vertices.length; i ++ ) {
                ...
                var A,B,C;
                var scale;
                if (CPK[geometryAtoms.elements[i].toLowerCase()] === undefined) {
                  if (CPK[geometryAtoms.elements[i][0].toLowerCase()] === undefined)
                    ...
                    C = 128 / 255;}
                      ...
                      scale = CPKsize[geometryAtoms.elements[i][0].toLowerCase()];
                    }
                }
                else {
                    A = CPK[geometryAtoms.elements[i].toLowerCase()][0] / 255;
                    B = CPK[geometryAtoms.elements[i].toLowerCase()][1] / 255;
                    C = CPK[geometryAtoms.elements[i].toLowerCase()][2] / 255;
                    scale = CPKsize[geometryAtoms.elements[i].toLowerCase()];
                }
                var color = colorscale.setRGB( A,B,C );
                var element = geometryAtoms.elements[ i ];

                var canvas = imageToCanvas( baseSprite, scale );
...
function imageToCanvas( image, scale ) {
    ...
    var width =128*scale; // controls size of balls
            ...
            if (CPK[geometry.elements[i].toLowerCase()] === undefined) {
              if (CPK[geometry.elements[i][0].toLowerCase()] === undefined)
                ...
                C = 128 / 255;}
                  ...
                  scale = CPKsize[geometry.elements[i][0].toLowerCase()];
                }
            }
            else {
                ...
                scale = CPKsize[geometry.elements[i].toLowerCase()];
            }
            var color = colorscale.setRGB( A,B,C );
...
function showDiv() {
    ...
    if (document.getElementById('filtershow').checked ) {
        $('#filters').show();
    }
    else {
        $('#filters').hide();
...
function filter(row) {
  ...
  for (i=1;i<row.length;i++) {
    ...
    var rangemin= document.getElementById(text+'min');
    var rangemax= document.getElementById(text+'max');
    if (row[i-1] <= rangemax.value && row[i-1] >= rangemin.value)
    {  }
    else {
      answer= false;}
  }
